...
var _ = Describe("LibraryRepository", func() {
	...
	It("refreshes stats", func() {
		...
		Expect(conn.NewQuery("select count(*) as count from folder where library_id = {:id} and missing = 0 and num_audio_files > 0").Bind(dbx.Params{"id": 1}).One(&foldersRes)).To(Succeed())
		Expect(conn.NewQuery("select ifnull(sum(num_audio_files + num_playlists + json_array_length(case when json_valid(image_files) then image_files else '[]' end)),0) as count from folder where library_id = {:id} and missing = 0").Bind(dbx.Params{"id": 1}).One(&filesRes)).To(Succeed())
