...
I also wanted both playback modes available: stream-only (no local copy, like the existing Internet Radio feature) and download-and-store (episode cached to disk, works offline, needed for real Subsonic-client compatibility). Subscriptions are shared/admin-managed, mirroring how Internet Radio stations work today (not per-user).

**Reference implementation I mirrored throughout:** the Internet Radio Station feature (`model/radio.go`, `persistence/radio_repository.go`, `db/migrations/20230115103212_create_internet_radio.go`, `server/nativeapi/radios.go`, `server/subsonic/radio.go`, `ui/src/radio/`) is the closest existing analog — an external-URL-based, admin-managed, non-scanned media resource — and I followed its patterns closely wherever they applied.
...
| Phase | What | Status |
|---|---|---|
| 1 | Data model, RSS refresh, native REST, minimal web UI | ✅ Done |
| 1.5 | Podcast discovery (search + regional top charts) | ✅ Done |
| 2 | Subsonic API, download-to-disk, real `stream.view` serving | ✅ Done |
| 3 | Retention enforcement, playlist integration, personal toggle, listened tracking | ✅ Done |
| 4.1 | Resume position ("in progress" tracking) + completion-based "played" | ✅ Done |
| 4.2 | Cross-channel "Up Next" / continue-listening queue | 📋 Planned |
| 4.3 | OPML import/export | 📋 Planned |

Legend: ✅ done and shipped · 🚧 in progress · 📋 planned, not started

---

## Architecture decision: how episodes stream through `/rest/stream.view`

`server/subsonic/stream.go`'s `Stream()` handler currently does one thing: `mf, err := api.ds.MediaFile(ctx).Get(id)`, then streams `*model.MediaFile` through `core/stream`. Podcast episodes are **not** MediaFile rows — I deliberately didn't synthesize fake ones. `model.MediaFile` is deeply entangled with the scanner/library/folder subsystem (e.g. `AbsolutePath()` needs a real `library_id`/`folder_id`, and every read joins against the `library` table), and forcing episodes into that shape would have caused more problems than it solved. Instead:

- Podcast episodes get their own model + repository (`model.PodcastEpisode`), no ID-prefix scheme needed — this codebase already has a precedent for polymorphic ID lookup: `model/get_entity.go`'s `GetEntityByID` tries each repository in turn until one succeeds (used today by `Download()`'s `model.GetEntityByID(ctx, api.ds, id)` call).
- In `Stream()`, try `ds.MediaFile(ctx).Get(id)` first (**zero change to the existing hot path** — no overhead for the 99.9% case of normal track streams). Only on `errors.Is(err, model.ErrNotFound)`, fall back to `ds.PodcastEpisode(ctx).Get(id)`. Simpler and more consistent with the codebase's existing conventions than introducing ID prefixes.
...
This means the web player needs **no** `isRadio`-style raw-URL bypass for episodes — `subsonic.streamUrl(episode.id)` works universally, simpler than the Radio precedent.
...
- `model/artwork_id.go`: add `KindPodcastChannelArtwork = Kind{"pc", "podcast_channel"}` to the `Kind` map (mirrors `KindRadioArtwork`), plus an `artworkIDFromPodcastChannel()` helper. Episodes reuse their parent channel's artwork (no separate per-episode art in Phase 1/2 — RSS episode-level images are a future stretch goal).
- Migration `db/migrations/<next-timestamp>_create_podcasts.go` (checked `db/migrations/` for the latest existing timestamp and bumped past it): creates `podcast_channel` and `podcast_episode` tables (the latter with `channel_id references podcast_channel(id) on delete cascade`, `unique(channel_id, guid)`, and indexes on `channel_id`/`download_status`/`publish_date`). Structure mirrors `20230115103212_create_internet_radio.go`.
...
- **Dependency:** added `github.com/mmcdole/gofeed` to `go.mod`. Real-world podcast feeds are inconsistent (missing GUIDs, six different date formats, `<itunes:duration>` as either `HH:MM:SS` or raw seconds, mixed RSS/Atom). Hand-rolling `encoding/xml` parsing would mean reimplementing gofeed's edge-case handling for no benefit — this is a case where the dependency clearly earns its place (same tier of justification as this project's existing use of `robfig/cron`).
...
- Wired into `core/wire_providers.go`'s existing `Set` directly (same place `playlists.NewPlaylists` is registered) — added `podcasts.New`.

### Scheduling
- `cmd/root.go`: added `schedulePodcastRefresh(ctx)`, copying `schedulePeriodicScan`'s shape — reads `conf.Server.Podcasts.Schedule`, calls `scheduler.GetInstance().Add(schedule, func() { podcastsService.RefreshAll(ctx) })`. Registered via `g.Go(schedulePodcastRefresh(ctx))` next to the existing scan scheduling call. Added `CreatePodcastsService(ctx)` to `cmd/wire_injectors.go` (mirrors `CreateScanner`).
...
### Progress events
- `server/events/events.go`: added `PodcastRefreshStatus` and `PodcastDownloadStatus` events (same `baseEvent`-embedding shape as the existing `ScanStatus`). `core/podcasts` broadcasts via `broker.SendBroadcastMessage` at each state transition, plus `events.RefreshResource{}` after mutations so React-admin list views auto-refresh (existing generic mechanism).

---

## Subsonic API surface (`server/subsonic/podcast.go`, new)

Replaced the `h501(r, "getPodcasts", ...)` line at `server/subsonic/api.go:229-230` with real registrations, gated behind `conf.Server.Podcasts.Enabled` the same way `EnableSharing` gates the share endpoints (`api.go:207-217` — `if enabled { h(...) } else { h501(...) }`):
...
- `episode.status`: maps `DownloadStatus` → spec vocabulary (`not_downloaded`→`skipped`/`new` depending on policy, `queued`/`downloading`→`downloading`, `downloaded`→`completed`, `error`→`error`, `deleted`→`deleted`).
...
Registered via `api.addPodcastRoutes(r)` in `server/nativeapi/native_api.go`, alongside `api.addRadioRoute(r)`.
...
- `ui/src/App.jsx`: `import podcast from './podcast'`, then `<Resource name="podcastChannel" {...(permissions === 'admin' ? podcast.admin : podcast.all)} />` and `<Resource name="podcastEpisode" {...podcastEpisode} />` (episodes aren't a separate sidebar item — same pattern as playlist tracks). **No `ui/src/layout/Menu.jsx` edit needed** — the sidebar is auto-generated from registered `<Resource>` elements with an `icon` prop (Radio's sidebar entry comes entirely from `radio/index.jsx`'s `DynamicMenuIcon`, not a Menu.jsx edit, so I followed the same pattern).
...
Model + migration + persistence + `core/podcasts` (refresh only, no download pipeline) + gofeed dependency + config (`Enabled`/`Schedule`/`DefaultDownloadPolicy`) + scheduler wiring + artwork reader + native REST (channel CRUD + refresh) + web UI (channel list/create/edit, read-only episode list). Episodes play via a temporary direct-URL player bypass (copy of the `isRadio` pattern) since `stream.go` doesn't branch yet. **Outcome:** I can subscribe to a feed as admin and play episodes in the Navidrome web UI. **Status: ✅ done.**

**Phase 1.5 ✅ — Podcast discovery (search + live regional top charts)**
Requiring myself (or anyone else using the server) to already have an RSS feed URL is a poor onboarding experience, so this phase added real discovery before Phase 2's heavier download-pipeline work:
...
- A shared `TopFeedsSuggestions` component (country guessed client-side from timezone, with a manual override) renders the live regional top-chart as one-click quick-add chips. Used in **two** places — the empty-state Podcasts list (first-run) *and* always-visible on the Create/search page itself — since a first version that only showed suggestions on the empty list became unreachable the moment I subscribed to anything.
**Outcome:** subscribing to a podcast is "search by name" or "pick a trending show for your region," always reachable via the Create page, not just requiring a raw feed URL on first run. **Status: ✅ done.**

**Phase 2 ✅ — Subsonic API + download-to-disk + real stream.view serving (unlocks Cirque)**
Finished config (`StorageFolder`/`DownloadConcurrency`/`MaxDownloadSizeMB`) + download pipeline (`go-pipeline`) + SSE progress events + all 7 Subsonic handlers + response structs + the `stream.go`/`Download()` branching logic + native REST download/delete actions + web UI download-status badges, and removed the Phase 1 player bypass now that `stream.view` handles every episode. **Outcome:** my actual goal — Cirque (Android/Windows) can subscribe, browse, download, and stream podcasts exactly like any other Subsonic server. **Status: ✅ done.**

**Phase 3 ✅ — Retention enforcement, playlist integration, personal UX polish**
**Status: ✅ done.** Three linked pieces:

1. ✅ **Retention enforcement** (`core/podcasts/retention.go`, new) — `RetentionCount`/`RetentionDays` already existed as columns on `PodcastChannel` (added in Phase 1) but were dormant; nothing read or acted on them yet. This phase added the actual `RunRetention(ctx)` enforcement, run per-channel after each scheduled refresh (same job as `schedulePodcastRefresh`) and available as an explicit trigger:
   ...
   - Retention skips any episode currently referenced by a playlist (see pinning rule below) — being in a playlist counts as "in use."

2. ✅ **Playlist integration, downloaded episodes only** — the piece I actually wanted this for. Podcast episodes can be added to regular playlists, but **only once downloaded** (`DownloadStatus == Downloaded`); stream-only/not-yet-downloaded episodes are rejected at add-time (both native REST and Subsonic `createPlaylist`/`updatePlaylist`). This restriction is what keeps the feature simple:
   - Schema: relaxed `playlist_track` to reference an episode as well as a `MediaFile`, reusing the polymorphic-ID pattern already established by `model.GetEntityByID` and `stream.go`'s MediaFile-then-PodcastEpisode fallback, rather than a full playlist rearchitecture.
   - Drag-to-reorder comes for free — it's the existing playlist position mechanism, agnostic to what the underlying entity is.
   - M3U export (`model/mediafile.go`'s `ToM3U8`, also used by playlist share links) writes local filesystem paths per line, with no concept of "stream from the server." Because only *downloaded* episodes can ever be in a playlist, every entry always has a real file on disk, so no special-casing is needed for the stream-only case. Kept a defensive fallback (skip the row if `Path` is unexpectedly empty) rather than assuming a file can never go missing.
...
4. ✅ **"Listened" indicator** (added after the above three, same phase in spirit) — episodes participate in the same per-user `annotation` table songs use (`play_count`/`play_date`), scoped to just play-tracking (no starring/rating — that would need an `average_rating` column podcast_episode doesn't have). `persistence/podcast_episode_repository.go`'s `withPlayAnnotation` left-joins it into `Get`/`GetAll`/`GetNewest`. Originally, `server/subsonic/podcast_stream.go`'s `streamPodcastEpisode` recorded a play as soon as a client started streaming, with no position/completion threshold — that placeholder was replaced by real completion detection in Phase 4 item 1 below. Web UI shows a checkmark column in the episode list. **Status: ✅ done.**
...
**Phase 4 — Resume position (✅ shipped), continue-listening queue, OPML import/export (still 📋)**

1. ✅ **Resume position ("in progress" tracking) + completion-based "played"** — the biggest usability gap for podcasts specifically: a 90-minute episode isn't finished in one sitting, and every replay used to start from zero; separately, "played" fired the instant a stream started rather than requiring the episode actually be finished. **Status: ✅ done**, but via a different, simpler design than originally scoped below (kept for context on what was considered and rejected):
   - **Actual design — reused the existing bookmark mechanism instead of a new `annotation.play_position` column.** The `bookmark` table (`createBookmark.view`/`getBookmarks.view`/`deleteBookmark.view`, the same primitive songs already use) was already fully generic at the schema/persistence level; the only hardcoded part was `GetBookmarks()` assuming a `MediaFile`. `model.Bookmark.Item` changed from a concrete `MediaFile` to `any`, `model.PodcastEpisodeRepository` gained `BookmarkableRepository`, and `server/subsonic/bookmarks.go` now type-dispatches (via `model.GetEntityByID`) between `MediaFile` and `PodcastEpisode` on create/list/delete. This means any client that already supports bookmarks for songs — Cirque included — gets podcast resume for free, with zero new endpoints.
   - **Completion detection**: `core/podcasts/podcasts.go` gained `RecordEpisodePosition(ctx, episodeID, positionMs) (justCompleted bool, err error)`, called from the bookmark-create path. It compares position against `episodeCompletionThreshold` (a `const`, currently 90%) of the episode's duration; once crossed (and the episode isn't already listened), it calls `IncPlayCount` — same shared method songs use, only the podcast-specific *call site* changed. The old `streamPodcastEpisode` call to `IncPlayCount` on stream-start was removed outright, along with its stale "simpler than tracking position" comment.
   - `persistence/podcast_episode_repository.go`'s `withPlayAnnotation` join was extended to surface `bookmark_position`; `toPodcastEpisode` in `server/subsonic/podcast.go` now populates `BookmarkPosition` on the Subsonic response, and (from a separate, earlier change) a `played` timestamp field too — together these give a client everything needed to render not-started / in-progress / completed without a new tri-state field.
   - Explicit `markPodcastEpisodeListened.view`/`Unlistened.view` and scrobble-submission (`scrobble.view submission=true`) both still work exactly as before, independent of position — only the passive "started streaming" signal was replaced.
   - **Known caveat, not fixable from this side**: `nd_source` attribution is read per-request, so a client now needs to send it on its bookmark/position-report calls too (not just `stream.view`) to keep that attribution once the plugin-dispatch point moved. Completion detection also depends on the client actually calling `createBookmark.view` near the end of an episode, not just deleting the bookmark on natural finish with no final position update.
   - *(Originally scoped, not what shipped)*: add `play_position` to `annotation`, dispatch through `media_annotation.go`'s `ReportPlayback()`. Rejected once the bookmark table turned out to already be structurally generic — reusing it meant zero new schema and immediate compatibility with any client already doing song bookmarks, instead of inventing a parallel Navidrome-only mechanism.
...
2. 📋 **Cross-channel "Up Next" / continue-listening queue** — today, episodes are only browsable one channel at a time (`ReferenceManyField target="channel_id"` in `PodcastChannelShow.jsx`). Every mainstream podcast app's home screen is a single unified feed of newest-unplayed episodes across every subscription — low marginal backend cost given `GetNewest` and the listened-tracking from Phase 3 item 4 already exist, but a materially different daily workflow (check one feed of what's new, not N channel pages).
   ...
   - Deliberately **not** added to the spec-compliant `getNewestPodcasts` Subsonic endpoint (real `getNewestPodcasts` has no "unlistened" concept in the spec) — keeping that endpoint spec-compliant/unchanged, and adding this as a native-only endpoint + web UI view, since it's a value-add specific to this fork rather than a protocol feature.
...
- `model/bookmark.go` (Phase 4.1, ✅ shipped, modify) — `Bookmark.Item` decoupled from `MediaFile` to `any`
- `model/podcast.go` (Phase 4.1, ✅ shipped, modify) — `PodcastEpisodeRepository` embeds `BookmarkableRepository`
- `persistence/sql_bookmarks.go`, `persistence/podcast_episode_repository.go` (Phase 4.1, ✅ shipped, modify) — generic ID-fetch split out of `GetBookmarks()`, per-repo typed assembly, `bookmark_position` surfaced in `withPlayAnnotation`
- `server/subsonic/bookmarks.go` (Phase 4.1, ✅ shipped, modify) — type-dispatch via `model.GetEntityByID` across MediaFile/PodcastEpisode
- `server/subsonic/podcast.go`, `server/subsonic/podcast_stream.go` (Phase 4.1, ✅ shipped, modify) — `BookmarkPosition` on responses, stream-start `IncPlayCount` removed
- `core/podcasts/podcasts.go` (Phase 4.1, ✅ shipped, new) — `RecordEpisodePosition`, `episodeCompletionThreshold` const (90%)
- `core/podcasts/` (Phase 4.2/4.3, still 📋) — `GetUnlistened` query, `ImportOPML`/`ExportOPML`
...
- Manual: subscribed to a real RSS feed via the web UI, confirmed episodes appear and play (Phase 1); then via a Subsonic API client — my actual Cirque app — confirmed `getPodcasts`, `createPodcastChannel`, `downloadPodcastEpisode`, and streaming a downloaded + a stream-only episode all work end-to-end (Phase 2).
