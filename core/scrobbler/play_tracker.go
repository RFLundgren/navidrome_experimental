	...
	case StateStopped:
		...
		if !params.IgnoreScrobble && player.ScrobbleEnabled {
			...
			if params.PositionMs >= threshold {
				...
				p.dispatchScrobble(ctx, mf, now, params.ClientName, params.Source, params.Origin, params.PlaybackMode, filtered)
			}
		}
		p.sessionsMu.Lock()
		info, getErr := p.playMap.Get(clientId)
...
func (p *playTracker) Submit(ctx context.Context, submissions []Submission) error {
	...
	for _, s := range submissions {
		...
		if err != nil {
			...
			if player.ScrobbleEnabled {
				p.dispatchScrobble(ctx, mf, s.Timestamp, client, s.Source, s.Origin, s.PlaybackMode, filtered)
			}
		}
	}
...
func (p *playTracker) dispatchScrobble(ctx context.Context, t *model.MediaFile, playTime time.Time, client, source, origin, playbackMode string, filtered bool) {
